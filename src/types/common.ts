...
// Based on @node-sc2/core/interfaces.d.ts
export interface Agent extends PlayerData {
    ...
    interface: { raw: boolean; };  // raw must be required, no undefined
...
export interface AgentSystem {
    ...
    interface: { raw: boolean; };  // Make interface required in AgentSystem
}
...
export interface Events {
    ...
    write: (event: { name: string; data: any; }) => void;
}
...
export interface DataStorage {
    ...
    addEarmark: (earmark: { name: string; minerals: number; vespene: number; }) => any[];
    getEarmarkTotals: (earmarkName: string) => { minerals: number; vespene: number; };
